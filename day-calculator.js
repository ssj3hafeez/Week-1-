function calculateDayInYear(date) {
    var splitDate = date.split('/'); // split a string into arrays//
    var year = Number(splitDate[0]); // outputs the years //
    var month = Number(splitDate[1]); // outputs the month //
    var day = Number(splitDate[2]); // outputs the date// 
    var febDays = daysInFeb(year); // number of days in Feb depeending on the year if its a leap year // 
    var DAYS_IN_MONTH = [31, febDays, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]; // days in the month in arrays
  
    // if statement that states if the year and month
